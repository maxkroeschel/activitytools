...
#'                               resting_range_limit = 20,
#'                               threshold_range_limit = 80,
...
# Function to calculate the position of the lower resting_range_limit/100
# part of the value range.
  resting_range_threshold.f <-
    function(x) {(max(x, na.rm=T)-min(x, na.rm=T))*resting_range_limit/100 +
                  min(x, na.rm=T)}
...
# Set the last candidate threshold matching the threshold_range_limit.
  max_seq <-
    max(3,quantile(i_activity[, get(act_ma)],
                   probs = (threshold_range_limit/100), na.rm =T))
